...
Image functions can work being invoked either from package,
...
Only this last item can be undefined value, others are guaranteed
to be valid objects.
...
that this extra information is desirable, loadExtras boolean value is used.
Example:
...
The code above loads and prints extra information delivered from a file.
...
'codecID' is a Prima-defined extra field, which is a index of codec
which loaded the file. Useful for explicit indication of codec on save request.
...
who apply only to particular frame. Those who are defined by Prima, are
enumerated above - loadExtras, loadAll etc. Only loadExtras and noImageData 
are applicable to
a frame, other governs the loading process. A codec also may define its
parameters, however it is not possible to know what parameter belongs to what
group. 
...
If desirable to peek into image, reading just type and dimensions, one
...
are set to image dimensions. Example:
...
Some information about image can be loaded even without frame loading - if a 
codec provides such a possibility. This is the only request that cannot be issued
on a package:

        $x-> load( $f, map => [], loadExtras => 1);

Since no frames are required to load, an empty array is returned 
upon success and array with one undefined value on failure.

=head2 Using Prima::Image descendants

If Prima needs to create an storage object, it is by default
Prima::Image, or a class name of an caller object, or a package
the request was issued on. This behavor can be altered 
using parameter 'className', which defined a class to be used 
for the frame.
...
Save requests also can be done with package syntax:
...
All information, that is found on object hash reference 'extras', is
...
that keys that were not recognized by codec just ignored, and invalid values
raise an error. 
...
selects explicitly a codec which is desired to handle an image. If a codec
...
is never extracted from objects, passed to 'images' array. 
...
either convert an image to an appropriate format or signals an error.
...
Profile key 'append' ( 0 by default ) triggers this behavior - if it's set,
...
always interestring to know, what part.
...
is image type that to be saved is not in that list, the image will be 
