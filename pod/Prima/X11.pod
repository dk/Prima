...
Runs Prima without X11 display intialized. This switch can be used for programs
...
native X API, it is rarely doen by applications. Instead, the user creates a
...
Also, classes and names are distingushed by the case: classes must begin with
...
latter is a per-widget assignmemt, and explicitly sets font to B<all> Prima
...
the whole class and another for the name, for the majority of these properies it
...
fonts to the nonscalable fonts only.
...
its format.  The idea behind it is that users can benefit from protable literal
...
extrenal informartion.  The indexed-color scheme represents pixel value as an
...
Prima woudln't work on some bizarre hardware. Currently, Prima knows how to
...
for example a top-level widget was reparenteted into another widget, but the
window manager is not aware or this yet.

The consequences of this, as well as programming gudances are described in
...
C<WM_DELETE_WINDOW> use is straighforward and needs no further attention,
...
representation, which results in sufficient latency when fonts with large span
...
Indentical to C<--visual> command-line argument.
