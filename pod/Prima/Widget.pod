...
Sometimes Prima needs to implicitly re-create the system handle of the widget.
The re-creation is not seen on the toolkit level, except for some repaints, but
under the hood, when it happens, Prima creates a whole new system resource when
the underlying system either doesn't have API to change a certaint property
during the runtime, or when such a re-creation happens on one of widget's
parent, leading to a downward cascade of children re-creation. 

The list of properties, that when changed, may tigger this event: L<owner>,
L<clipOwner>, L<syncPaint>, L<clipOwner>, L<layered>, L<transparent>
