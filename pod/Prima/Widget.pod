...
position and size. The notifications occur irrespective to whether the
...
with its parent. The widget become parent-less, or, more strictly
...
functions, correspondingly. These return boolean values corresponding to the
...
provide the navigation are available and called C<next_tab()> and
...
enough memory, then widget draws in the usual manner. One can
check whether the buffering request was granted by calling the
C<is_surface_buffered> method.
...
C<::geomSize>. The properties are run-time only, and behave differently
...
explicit size is additionally sets the advised size in case the widget is
...
Automatically set to 0 if the C<skin> property is explicitly set.
...
A generic scalar not used in C<Prima::Widget> but designated to select the
visual style of a widget, where the interpretation of the property value will
be up to the widget class itself. Many of the toolkit widgets implement two
skins, C<classic> and C<flat>. Also, does not repaint the widget on change, but
again, many of the toolkit widgets do that.

If C<ownerSkin> is 1, returns the skin of the owner.  When set to undef, sets
the C<ownerSkin> property to 1, otherwise to 0.  Note: this is therefore not an
symetric property, as a C<< $self->skin($self->skin) >> is not an idempotent
call.
...
Returns true if the C<buffered> property is set and the buffering request was granted.
The value is only valid inside C<begin_paint/end_paint> bracket, and is always false
otherwise.
