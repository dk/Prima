...
=item alpha none|split|blend

Affects how to treat aplha channel bits, if any.

If set to C<blend>, mixes the alpha channel with C<background> color.

If set to C<split> (not applicable for C<Prima::Icon>), adds a 8-bit grayscale image
to C<extras> that contains the separate alpha channel under C<alpha> key. 

Default: blend

Note: PNG alpha channel is 8-bit, but Prima's is 1-bit only. The loaded downsamples the
alpha channel, if any, when loading to C<Icon>.
...
8-bit grayscale image with alpha channel, if found
...
=item transparent_colors [COLOR], transparency_table [INT]

When image contains transparent colors, returns either array of palette indexes
(C<transparency_table>) in 0-255 range, for paletted images. Or an array of
colors in RGB form for 24-bit images (C<transparent_colors>).

Not applicable for save.

Note: support for transparency is not symmetric; codec cannot automatically
save the transparency info read from PNG files. See C<transparent_color> and
C<transparent_color_index> for manual save of one transparent color.
...
For GIF animation see L<Prima::Image::AnimateGIF>.
