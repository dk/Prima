...
Same as C<Prima::matrix> but explicitly binds to a drawable objects, so that
all changes to the matrix are immediately reflected in the drawable.
...
Sets matrix to C<Prima::matrix::identity>, or (1,0,0,1,0,0)
