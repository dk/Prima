...
=item dwm_blur enable => $BOOL, transition_on_maximized => $BOOL, mask => $Prima::Region

This effect is implemented only on Win32 version 7 or later, using Win32 API call 
L<DwmEnableBlurBehindWindow|http://msdn.microsoft.com/en-us/library/windows/desktop/aa969508>.
See the MS reference for details.

To determine whether this effect is available or not, use C<< $::application->get_system_value(sv::DWM) >>
call. See F<examples/dwm_blur.pl> for example of use.
