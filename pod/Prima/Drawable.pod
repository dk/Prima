...
Another method is to use the C<::palette> property. It works by inserting
...
=for html <p>
<figure>
<img src="https://raw.githubusercontent.com/dk/Prima/master/pod/Prima/aa2.gif">
...
functions is listed, which is also suitable for use in the C<lineEnd>
property.
...
(see below).  If a constant value is needed to be recovered, use the C<fp::builtin>
function that would return the constant from the array. There are also two
...
the next substitutions use it before trying to look into the passive list. Since
...
=item draw_underline TEXT, X, Y, %OPT

Given a text string, draws the dashed line that underlines the TEXT in such a
way so that the glyphs with descent parts, such as g,p,j,etc, will not be
crossed by the line.

TEXT is either a character string or a C<Prima::Drawable::Glyphs> object
returned from C<text_shape>, or a C<< Prima::Drawable::Glyphs->glyphs >> string
of glyphs. In the latter case some wrapping options are not applicable.
If TEXT is a character string, the %OPT hash is fed to a C<text_shape> call
internally.

=for podview <img src="puppies.gif" cut=1 title="Custom text underline">

=for html <p><img src="https://raw.githubusercontent.com/dk/Prima/master/pod/Prima/puppies.gif"><!--
...
Line plotting C<$OPT{effect}> can be either C<line> (default), or one of
the effects defined by the L<Prima::Drawable::Wave> plotter. See there for more
details.

Context used: color, backColor, font, lineWidth, lineEnd, linePattern

See also: C<render_underline>
...
FOREGROUND is undefined, the current color is used. BORDER_WIDTH is 1 pixel if
omitted.
...
See L<https://en.wikipedia.org/wiki/Non-uniform_rational_B-spline>.
...
see L<Unicode Standard Annex #9 | http://unicode.org/reports/tr9/tr9-22.html> 
...
=item add_region REGION [RGNOP = rgnop::Intersect]

Combines the existing drawable region with REGION using RGNOP logical operation.
Useful when both drawable region and REGION can be C<undef>.
...
Treats keys C<region> and C<matrix> in a special manner: doesn't replace the existing properties
but combines them.
...
Same as C<Prima::Font::has_color>

=item prelight_color COLOR, [ COEFFICIENT = 1.05 ]

Given a COLOR value, returns another color that visually can be regraded as its
highlighted version. The amount of light (or dark, depending on the color), can
be specified by the COEFFICIENT.
...
If set and the glyph has color information, reports also the color information
of individual layers with the C<ggo::SetColor> command. In this case, the
command is followed by 2 integers, where only the first is used to set the
foreground color as a 24-bit RGB value. The value of -1 means to use the
...
The C<ggo::> commands are:
	...
	ggo::SetColor - set foreground color
...
are on the resulting image's lower and left boundaries. Also the image
size corresponds to the polygon size.

The filling mode can be specificed with the C<mode> option, or the current
C<fillMode> will be used. Note that the option only accepts the C<fm::Winding>
and C<fm::Alternate> constants, and ignores C<fm::Overlay>.

Return three scalars: first two are the X and Y polygon offsets, and
the third is the image itself.

=item box BOOLEAN

If set, instead of polyline vertices, calculates the box extents of the
polyline (either original or after the matrix transform, depending on whether
the C<matrix> option was supplied or not), and returns 4 numerics for left,
bottom, width, and height of the box enclosure.
...
Note: for speed, Prima::array->new_cmatrix can be used.
...
=item render_underline \@GLYPHS, %OPTIONS

Given a text string as a set of @GLYPHS, builds a set of numerical values that
can be used to represent the dashed line that would underline the string in
such a way so that the glyphs with descent parts, such as g,p,j,etc, will not
be crossed by the line.

GLYPHS is either a C<Prima::Drawable::Glyphs> object returned from
C<text_shape>, or a C<< Prima::Drawable::Glyphs->glyphs >> string of glyphs.

The returned array consists of pairs of X,Y-coordinates, where the first
coordinate marks the beginning, and the second the end of an individual dash.
...
assume this underline thickness, or use the current font's predefined value

=item position

assume this underline position, or use the current font's predefined value

=item bevel

assume how many pixels line endings protrude over the vertices used to plot an underline,
or use the current line pattern bevel. Line patterns may provide different line endings for start
and end of a line, however this setting only accepts one value for both.

=back

See also: C<draw_underline>
...
=item text_split_lines TEXT

A convenience wrapper for splitting TEXT into lines separated by newlines.
TEXT can be a character string, in which case this method is equivalent to a
C< split "\n", $TEXT > call. Or a reference to an object, such as f.ex.
C<Prima::Drawable::Block>, where the object uses its own algorithm to split
itself.
