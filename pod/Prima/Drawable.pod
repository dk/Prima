...
There is defined a set of constants to apply a constant source and destination alpha to whe
...
cluster but are represented by a two glyphs C<"A"> and <"`">. The grave glyph
...
These tables can be manipulated laters, and are respected by C<text_out> and
C<get_text_width>.

=item language STRING = undef

When set, shaping process can take in the account the language of the text.
F.ex. text C<"ae"> will be shaped as a single glyph I<E<aelig>> for the Latin
language, but not for the English.

=item level INTEGER = ts:XXX

Select the shaping (i.e. text to glyph conversion) level, how the system should treat the input text
...
than 255 are treated as chr(255).  Reordering never happens, kerning and
ligation never happens, returns glyph indexes in a 1:1 mapping to each
codepoint.
...
are filled with the proper character offsets, effectively making it a visual to
logical map since number of glyphs will always be equal to number of characters
in TEXT, because ligation never happens here (except when text contains unicode
directional characters such as isolates etc).
...
When polyfont is active and font substitution is desired, filters only fonts
...
additionally RTL codepoints are reversed (depending on direction context).
...
characters, so that reordering is needed to cancel this. In theory the caller
...
If unset, the text direction is taken from C<Prima::Application::textDirection>.

=item skip_if_simple BOOLEAN = false

When set, checks whether shaping result is identical to the input, in a sense
