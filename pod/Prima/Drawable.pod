...
The example below fills a rectangle with a gradient calculated between a
background color and its pre-lighted variant.

    $canvas-> gradient_bar( $left, $bottom, $right, $top, {
  	palette  => [ $self->prelight_color($back_color), $back_color ],
    });

Here are keys understood in the gradient request:
...
=item spline @VERTICES

Serving same purpose as C<poly> but vertexes are projected first to a quadratic
Bezier curve using L<render_spline> and current value of C<splinePrecision>. The
resulting polyline is treated as C<poly>.

=item vertical BOOLEAN

Only used in L<gradient_bar>, to set gradient direction.

=back

See also: L<gradient_bar>, L<gradient_realize3d> .
...
Additionally, Prima implements extra features for compositing on images outside the
begin_paint/end_paint brackets. It supports the following 12 Porter-Duff operators:

=for podview <img src="rop2.gif" cut=1 >

=for html <p><img src="https://raw.githubusercontent.com/dk/Prima/master/pod/Prima/rop2.gif"><!--
...
and set of constants to apply a constant source and destination alpha to override the
...
To override the alpha channel(s) combine the rop constant using this formula:
...
In addition to that, C<rop::AlphaCopy> operation is available for accessing
alpha bits only.  When used, the source image is treated as alpha mask, and
therefore it has to be grayscale.  It can be used to apply the alpha bits
independently, without need to construct an Icon object.
...
C<fill_spline>, C<poly_bezier>, and C<fill_poly_bezier> calls. In other words,
determines smoothness of a curve. Minimum accepted value, 1, produces straight
lines; maximum value is not present, though it is hardly practical to set it
...
=item fill_poly_bezier \@KNOTS

Fills a polygonal area defined by a curve, projected by applying 
set of cubic Bezier curves based on set of KNOTS. Number of knots
should be 1 plus 3 timer number of curves, where last point of
a curve segment will be also first point of the next curve segment.

KNOTS must present an array of integer pair in (X,Y) format.
Example:

   $d-> fill_poly_bezier([ 0, 0, 0, 100, 100, 100, 100, 0]);

Context used: color, backColor, fillPattern, rop, rop2, splinePrecision

Returns success flag; if failed, C<$@> contains the error.

See also: poly_bezier, splinePrecision, render_poly_bezier
...
=item fill_spline \@VERTICES

Fills a polygonal area defined by a curve, projected by applying 
quadratic Bezier curve based on set of VERTICES.
VERTICES must present an array of integer pair in (X,Y) format.
Example:
...
=item gradient_bar X1, Y1, X2, Y2, GRADIENT

Draws a filled rectangle within (X1,Y1) - (X2,Y2) extents using a gradient (see L<Gradients>).

Context used: splinePrecision, fillPattern, rop, rop2

=item gradient_ellipse X, Y, DIAM_X, DIAM_Y, GRADIENT

Draws a filled ellipse with center in (X,Y) and diameters (DIAM_X,DIAM_Y) using a gradient (see L<Gradients>).

Context used: splinePrecision, fillPattern, rop, rop2

=item gradient_sector X, Y, DIAM_X, DIAM_Y, START_ANGLE, END_ANGLE, GRADIENT

Draws a filled sector with center in (X,Y) and diameters (DIAM_X,DIAM_Y) from START_ANGLE to END_ANGLE (see sector())
using a gradient (see L<Gradients>).

Context used: splinePrecision, fillPattern, rop, rop2

=item gradient_realize3d BREADTH, GRADIENT
...
L<gradient_bar> uses this information to draw a gradient fill, where
...
=item poly_bezier \@KNOTS

Draws a polygonal area defined by a curve, projected by applying 
set of cubic Bezier curves based on set of KNOTS. Number of knots
should be 1 plus 3 timer number of curves, where last point of
a curve segment will be also first point of the next curve segment.
...
Example:
   ...
   $d-> poly_bezier([ 0, 0, 0, 100, 100, 100, 100, 0]);

Context used: color, backColor, linePattern, lineWidth, lineEnd, rop, rop2

Returns success flag; if failed, C<$@> contains the error.

See also: fill_poly_bezier, splinePrecision, render_poly_bezier.
...
Draws a quadratic Berizer curve defined by set of VERTICES points.
VERTICES must present an array of integer pair in (X,Y) format.
...
=item render_poly_bezier KNOTS, [ PRECISION ]

Renders set of cubic Bezier curves from set of VERTICES to a polyline with given precision.
...
The method is internally used by C<poly_bezier> and C<fill_poly_bezier>, and is provided
for cases when these are insufficient.

=item render_spline VERTICES, [ PRECISION ]

Renders quadratic Bezier curve from set of VERTICES to a polyline with given precision.
The method can be called as static, i.e.  with no object initialized. PRECISION
integer, if not given, is read from C<splinePrecision> property if the method
was called on an alive object; in case of static call, default value 24 is
used.
