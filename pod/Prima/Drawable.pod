...
in the pattern as the curently selected C<backColor>, and 1s as C<color>. In
addition in the render mode, will respect C<rop2> when set to C<rop::NoOper>,
treating 0s as fully transparent pixels, and the C<alpha> property. Note:
Drawing over monochome bitmap or image will not respect its C<rop> on Win32.
...
Ignores color and backColor, just uses the tiles and the current C<alpha> value.

=item Icon

Ignores color, backColor, and alpha, uses the alpha pixel values from the icon's mask.
