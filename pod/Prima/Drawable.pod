...
can use it.  It can be turned on by calling
...
antialiased edges. Arcs, lines, paths, all of them manifest adjacent pixels
being renderes with half-tones.

=item *

C<alpha()> property will be used. All the primitives above would use it.
...
that had its coordinates automatically rounded to (5,5,10,10), now will rended
...
When antialiasing mode is on, sets alpha component of the brush, where 0 is fully transparent
