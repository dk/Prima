...
Takes a fill pattern represented by one of C<fp::XXX> constant, an array of 8 bytes, or an image (or icon);
same syntax as in C<fillPattern>.

Uses %OPTIONS to generate a new rectangular pattern that can be used in C<fillPattern> property.

Specifically respects C<preserveType> property and if sets, changed the resulting pattern type back
to the original type. In case or ARRAY or INDEX fillPattern, always generates a C<im::BW> image, so
it can be used both in C<rop2> transparent and opaque modes, line the original pattern.

Options:

=over

=item fill COLOR, mask COLOR

If C<margin> is used, fills pixels with this color.
C<mask> is used to fill mask pixels, if the image is an icon.
