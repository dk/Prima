...
Returns a new antialiasing surface object. See L<Prima::Drawable::Antialias> for usage and details.
...
or not.  Note that the drawable might not necessarily have an alpha channel,
for example a normal RGB image is capable to be painted on wil alpha while not
...
if integer results are needed, the results are transformed as to integers using
...
It is possible to send both text as TEXT and its shaped representation as GLYPHS.
