...
Prima uses own plotting mechanism in L<Prima::Drawable::Path> to produce all
primitive shapes, including lines. The way lines are plotted is governed by
...
There are 3 predefined C<le::> constants
...
primitives that plots a line cap, tranformed with respect to the current line
tangent and width. Details of the descriptor syntax are given below, but there are
also non-hardcoded line end constants defined by this method, that one can use
as well:

=for podview <img src="lineends2.gif" cut=1>

=for html <p><img src="https://raw.githubusercontent.com/dk/Prima/master/pod/Prima/lineends2.gif"><!--
...
These descriptors could also be arbitrarily transformed (f ex scaled):
...
and individual commands accept arbitrary number of vertices, but no less than 1 for C<line>,
...
For example, this is a definition of C<le::Arrow>:
...
See also: C<Custom line end styles>.

=item lineEndIndex INDEX, VALUE

Same as C<lineEnd> except only addresses a single line ending style.

Allows VALUE to be C<undef> for indexes 1,2,3; depending on the index, the line
style will be different ( see more in L</lineTail>, L</arrowHead>, L</arrowTail> ).

Allows special negative INDEX values, that behave differently if the line end
style is C<undef>: while positive INDEX queries may return C<undef>, or
possibly affect neighbor indexing (if these are, in turn, C<undef>s), negative
INDEX never return C<undef> on get-calls, and never affect neighbor styles on
set-calls.

The following lookup rules are used if a line end style is undef:

  #0 - can never be undef
  #1 - if undef, same as #0
  #2 - if undef, same as #0
  #3 - if undef, same as #1, and if it also is undef, then same as #0
