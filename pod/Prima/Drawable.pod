...
Selects a clipping region applied to all drawing and painting routines.  The
...
must present an array of integer pair in (X,Y) format.  If the first and the
last vertices point to the same point, draws closed spline shape ( Note - by
adding degree minus two points to the set; this is important if C<weight> or
C<knots> are specificed).
...
L<Prima>, L<Prima::Object>, L<Prima::Image>, L<Prima::Region>, L<Prima::Drawable::Path>
