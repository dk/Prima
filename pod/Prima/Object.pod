...
Building Prima classes are meant to be completely identical to standard perl OO
model. F.ex. to subclass a new package, a standard
...
F ex defining C<MyRole::init> won't have any effect where MyRole is not the
first base class (and things will explode badly if it is).

In a very special case where MyRole I<needs> to have methods that overload Prima
core, XS-implemented menthods, a special technique is used:
...
Do not subsclass MyRole from Prima objects.
...
are up in the hierarchy. Instead, the first parameter to these method is an
...
If you know L<Moose> standard syntax C<around>, this is exactly the same idea.

=item *

In the desendant class, inherit from MyRole normally, but in addition to that
explicitly call to overload its special methods:
