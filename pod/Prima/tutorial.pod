...
The toolking comes with a POD viewer program C<podview> that can be
incorporated into an application. This is rather straighforward as you can write
...
C<open_help> can also take a standard C<< L<link> >> syntax so for example
...
images in the perl documentation alright, and so does Prima, however both using
different syntax.
