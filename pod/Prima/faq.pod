...
It is possible to run things in parallel by calling the eveng processing
...
only viable if calculation can be quantized into relatively short time frames.
The general solution would be harder to implement and debug, but it scales
...
asyncronously read data passed through the pipe.
