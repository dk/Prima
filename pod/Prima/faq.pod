...
with any other internal event loop implementation along with Prima but the
results might be scary. There are a few points to note:
...
You have to not call C<run Prima> but instead call L<AnyEvent>'s condition variable
C<recv> function.

=item *

You have to use C<$::application-E<gt>yield> in an C<AnyEvent-E<gt>timer> object to
allow for the Prima UI to update periodically.

=back

See full example in F<examples/socket_anyevent.pl> .
