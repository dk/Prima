...
Prima Image Subsystem - How To Write A Codec
...
but bindings come later. Let us imagine we work with an imaginary 
...
Simpiest open_load() returns non-null pointer - it's enough to report 'o.k'
...
It's result will be available in PImgLoadFileInstance-> instance,
...
=head2 Writing to PImage-> data

As mentioned above, Prima insists on keeping it's image data
in 32-bit aligned scanlines. If libduff allows reading from 
file by scanlines, we can use this posiibility as well:
...
Here ends the simple part. You can skip down to "Register with 
image subsystem" part, if you want it fast.
...
way to keep picture worthy of it's name. Note the BYTEORDER symbol
...
all it's bits, codec can provide it in a smart way. Old code
will work, but will eat memory and time. A flag 
PImgLoadFileInstance-> noImageData is indicating if image data
...
Newly introduced macros pset_i is a convenience operator, 
assigning integer (i) as a value to a hash key, given as a
first parameter - it becomes string literal upon macro 
expansion. Hash used for storage is a lexical of type HV*.
Code 
...
anyway useful. From perl code, Image has a hash reference 'extras'
on object, where comes all this stuff. Codec can report also
such data, storing it in PImgLoadFileInstance-> frameProperties.
...
High level code can specifies whether of not the extra 
information should be loaded. This behavior is determined by
flag PImgLoadFileInstance-> loadExtras. Codec may skip this 
flag, the extra information will not be returned, even if
PImgLoadFileInstance-> frameProperties was changed. However, 
it is advised to check for this flag, just for a speed-up.
All keys, possibly assigned to frameProperties should
be enumerated for high-level code. These strings should be 
represented into char ** PImgCodecInfo-> loadOutput array.
...
mask stored in file, it might change object in the way
it fits better. Mask is stored on Icon in a -> mask field.
...
open_load() and close_load() are used as a brackets for load requests,
...
PImgCodecInfo-> canLoadMultiple claims that it cannot load
...
information, of whetever kind. It have to be stored into the hash
PImgLoadFileInstance-> fileProperties, to be filled once on
open_load(). The only exception is PImgLoadFileInstance-> frameCount,
...
PImgLoadFileInstance-> profile is a hash, that contains these
...
explain the load error, otherwise user gets common "Loading error"
string. To do so, error message is to be copied to 
PImgLoadFileInstance-> errbuf, which is char[256].
Note that returning a null value from open_load() means
generally that a codec and the file given do not match; 
image subsystem preceeds to call up other codecs until suitable one is
found. Therefore the situation when codec can't load a file isn't that fatal,
it just means that other codecs will be probed. Otherwise, if codec is sure that
the file format is matched and no codec will be capable of reading the file,
it can stop the processing by setting PImgLoadFileInstance-> stop flag to C<true>.
On that condition it is advisable to fill PImgCodecInfo-> errbuf also.
This flag is useful also to report a misconfiguration on open_load() stage 
( the information is accessible through PImgCodecInfo-> extras), but again, after
assuring that a file can be handled correctly.

On an extreme severe error a codec may call croak(),
...
codec calls some Prima code with issues croak(). 
Unfortunately it is untrappable, - at least without calling perl functions. 
Understanding that that behavior is not acceptable in some cases, 
it is still under design.

=head1 Multiple-frame load

In order to indicate that codec is ready to read
multiframe images, it have to set PImgCodecInfo-> canLoadMultiple
flag to true. This only means, that codec should respond to the
PImgLoadFileInstance-> frame field, which is integer that
can be in range from 0 to PImgLoadFileInstance-> frameCount - 1.
It is advised that codec should change the frameCount from
it's original value -1 to actual one, to help Prima filter range
requests before they go down to the codec. The only real problem that
may happen to the codec which is strongly unwilling to initialize
frameCount, worth an explanation, which is following.
...
typical saving code and highlighted differencies from load.
...
are defined in PImgCodecInfo-> saveTypes. Prima converts image
to be saved into one of these formats, before actual save() call
takes place.
Another boolean flag, PImgSaveFileInstance-> append
is summoned to govern appending or rewriting of a file, but
this functionality is under development. It's current value
is a hint, if true, for a codec not to rewrite but rather
apeend frames to an existing file. Due to increased
...
Finally, the code have to be registered. It's not as illustrative 
...
The result of init() is stored into PImgCodec-> instance, and
info into PImgCodec-> info. If dynamic memory was allocated
...
This procedure can register as many codecs it wants, 
...
As mentioned before, if an underlying Prima code calls croak(), then the loading or saving
process is stopped on the spot, with all beauties and unability to recover. 
Currently this can happen if Image-> create_empty() request fails because of memory lacks,
and high-level code was wrapped with eval{} block. These problems can't be solved just
by image subsystem, all croak() calls should be replaced to something more reasonable.
