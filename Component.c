...
static Bool free_private_posts( PostMsg * msg, void * dummy )
{
   sv_free( msg-> info1);
   sv_free( msg-> info2);
   free( msg);
...
static Bool free_queue( PEvent event, void * dummy) { free( event); return false; }

static Bool free_reference( Handle self, void * dummy) {
...
Component_done( Handle self)
{
   ...
   for (; var refs != nil;)
   {
      ...
      obj = hv_iternextsv( var refs, &k, &l);
...
void
Component_attach( Handle self, Handle objectHandle)
{
   if ( objectHandle == nilHandle || !kind_of( objectHandle, CComponent)) return;
   if ( var refs == nil) {
      HV * obj = ( HV*) SvRV( var mate);
      if ( SvTYPE( obj) != SVt_PVHV) {
         croak("RTC0040: Component attach failed");
         return;
      }
      var refs = newHV();
      hv_store( obj, "__CREFS__", 9,  newRV_inc(( SV *) var refs), 0);
   }
   {
      char key[ 20];
      _ltoa( objectHandle, key, 10);
      hv_store( var refs, key, strlen( key),
                newRV_inc( SvRV((( PAnyObject) objectHandle)-> mate)), 0);
   }
}

void
Component_detach( Handle self, Handle objectHandle, Bool kill)
{
   if ( objectHandle == nilHandle) return;
   if ( kill) Object_destroy( objectHandle);
   if ( var refs != nil)
   {
      char key[ 20];
      _ltoa( objectHandle, key, 10);
...
Component_set_name( Handle self, char * name)
{
   ...
   if ( var stage == csNormal) my update_delegator( self);
}
...
Component_set_delegate_to( Handle self, Handle delegateTo)
{
   ...
   if ( delegateTo) {
      ...
      while ( next && (( Handle) next != delegateTo)) next = ( PComponent) (( PComponent) next)-> owner;
      if ( next == nil) {
...
ForceProcess:
      ...
      switch ( event-> cmd & ctQueueMask) {
         ...
         case ctSingle:
            ...
            if ( list_first_that( var evQueue, ( PListProc) find_dup_msg, (void*) event-> cmd) >= 0)
               break;
         default:
            list_add( var evQueue, ( Handle) memcpy( malloc( sizeof( Event)), event, sizeof( Event)));
      }
   } else if ( var stage > csNormal && var stage < csDead) {
      if (( event-> cmd & ctQueueMask) == ctPassThrough) goto ForceProcess;
   }
   return ret;
}


Bool Component_can_event( Handle self)
{
   return var stage == csNormal;
}

void Component_clear_event( Handle self)
{
   my set_event_flag( self, 0);
}

void Component_push_event( Handle self)
{
...
{
   if ( !var evStack || var evPtr <= 0) croak("RTC0042: Component::pop_event call not within message()");
   return var evStack[ var evPtr--];
}

void Component_set_event_flag( Handle self, Bool eventFlag)
{
   if ( !var evStack || var evPtr <= 0) croak("RTC0043: Component::eventFlag call not within message()");
   var evStack[ var evPtr - 1] = eventFlag;
}

Bool Component_get_event_flag( Handle self)
{
   if ( !var evStack || var evPtr <= 0) croak("RTC0044: Component::eventFlag call not within message()");
...
static Bool oversend( PEvent event, Handle self) {
   my message( self, event);
   free( event);
   return false;
}

void Component_handle_event( Handle self, PEvent event)
{
#undef dyna
...
void Component_update_sys_handle( Handle self, HV * profile) {};
void Component_on_create( Handle self) {}
void Component_on_destroy( Handle self) {}
void Component_on_postmessage( Handle self, SV * info1, SV * info2) {}

HV *
Component_get_dynas( Handle self) {
   return ( HV*) SvRV( *hv_fetch(( HV*) SvRV( var mate), "__DYNAS__", 9, 0));
}

HV *
Component_get_delegators( Handle self) {
   return ( HV*) SvRV( *hv_fetch(( HV*) SvRV( var mate), "__DELEGATORS__", 14, 0));
}


void Component_update_delegator( Handle self)
{
    HV * profile;
...
void
Component_set_dyna_method( Handle self, char * methodName, SV * container, SV** variable)
{
   SV ** psv;
   ...
   container = ( SvROK( *psv) && ( SvTYPE( SvRV( *psv)) == SVt_PVCV))
      ? newSVsv( *psv) : nilSV;
   if ( is_hash) hv_delete( profile, methodName, len, G_DISCARD);
   if ( assign)
   {
      profile = ( HV*) SvRV( *hv_fetch(( HV*) SvRV( var mate), "__DYNAS__", 9, 0));
