...
skipto( ScanlinePtr* scan, int x, Bool subsample_last_pixel)
{
	...
	for ( i = 0; i < AAY; i++) {
		...
		while ( p != last ) {
			...
			if ( subsample_last_pixel) {
				register int x1 = p->x;
				register int x2 = p[1].x - 1; /* tag:RIGHTMOST, see that comment below */
...
aafill_init( NPoint *pts, int n_pts, int rule, Rect clip, PAAFillRec ctx)
{
	...
	clip.right++; /* poly_poly2points always returns that extra pixel
	since it is may be needed for superimpose_outline() later on.
	The AA algorithms never paints that pixel; also see the comment above with tag:RIGHTMOST */
...
aafill_next_scanline( PAAFillRec ctx, Byte *map)
{
	...
	if ( ctx-> saved_x >= 0 ) {
		...
		if ( map && ctx->saved_x >= 0 && ctx->saved_x < ctx->maplen ) map[ctx->saved_x] = 1;
		ctx-> saved_x = -1;
